...
MULTICLASS_DATA_PATH = "C:\\Users\\Kellie\\Desktop\\Machine_Learning\\Multiclass_DNA"
NEGATIVE_DATA_PATH = "C:\\Users\\Kellie\\Desktop\\Machine_Learning\\Negative_DNA"
positive_training_path = "C:\\Users\\Kellie\\Desktop\\Machine_Learning\\500_positive_training.tsv"
negative_training_path = "C:\\Users\\Kellie\\Desktop\\Machine_Learning\\500_negative_training.tsv"

class NeuralNetwork(object):
    training_data_types = ['intron', 'alu', 'sine', 'exon', 'mitochondrial']
    NEGATIVE_CLASSIFICATION_LABEL = 0
    POSITIVE_CLASSIFICATION_LABEL_COUNTER = len(training_data_types) # Number of human DNA labels
  ...
  #          y_train_copy = y_train_copy + [index] * min_num_samples # equal number of negative and positive training samples
    ...
    def add_output_layer(self, activation_function=tf.nn.softmax, classification="binary"):
        ...
        if classification == "binary":
            ...
            #  Multiclassification, add nodes equal to number of classification labels from sum of positive + negative labels
            num_nodes = NeuralNetwork.POSITIVE_CLASSIFICATION_LABEL_COUNTER + 1 # Adding 1 for single non-human label
...
def main():
    ...
    # First load all positive and negative training data into neural network
    # Positive training data:
    # introns (1,019,625 total positive training sequences), Alu elements (3,963,576 positive training sequences)
    merged_positive_data_file_path = os.path.join(MULTICLASS_DATA_PATH, "multiclass_merged_training_data.tsv")
    model1.load_data(input_file=merged_positive_data_file_path, training_type="positive", data_type="training")
...
#        model1.load_data(input_file="all_negative.tsv", training_type="negative", data_type="training")
    ...
    # Load negative training data - subsampled non-human DNA
    negative_data_file = os.path.join(NEGATIVE_DATA_PATH, "all_negative.tsv")
    model1.load_data(input_file=negative_data_file, training_type="negative", data_type="training")

    # Equalize training data lengths to be minimum length for all classifactions
    model1.equalize_training_data()

    # Load testing data sets
    model1.load_data(input_file=positive_training_path, training_type="positive", data_type="testing")
    model1.load_data(input_file=negative_training_path, training_type="negative", data_type="testing")
...
#        num_hidden_neurons_list = [200, 267, 300, 400, 500]
    num_hidden_neurons_list = [500]
    for hidden_neuron_num in num_hidden_neurons_list:
        print("hidden neuron number is: ", hidden_neuron_num)
        model1.build_nn(num_hidden_layers=1, num_hidden_neurons=hidden_neuron_num, activation="relu")
        model1.print_validation_loss_accuracy()
        model1.predict_unknown_datasets()
        model1.convert_classification_labels()
        TP, TN, FP, FN = model1.get_confusion_matrix(model1.y_test, model1.predictions)
...
if __name__ == '__main__':
    ...
    # Clean up memory after creating large models so that no leftover models interfere when rerunning script
    gc.collect()
